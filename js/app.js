...
async function startAudio() {
  ...
  try {
    ...
    // El navegador puede requerir una interacción adicional.
  }
  updateMusicButton();
}
...
function updateCountdown() {
  ...
  if (now >= end) {
    finishCountdown("¡Gracias por jugar este partidazo! ⭐");
    return;
  }

  if (now >= start) {
    finishCountdown("¡Hoy se juega! 🇦🇷⚽", "¡El partido ya empezó!");
    return;
  }
  ...
  else countdownStatus.textContent = `Faltan ${days} días para el partidazo.`;
}
